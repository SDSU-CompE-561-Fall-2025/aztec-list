...
- **MCP server (`mcp` SDK).** `src/app/mcp_server/server.py` is a FastMCP server exposing marketplace tools (`search_listings`, `semantic_search_listings`, `get_listing`, `list_categories`) that reuse `listing_service` (no duplicated logic). Run it over stdio with `uv run python scripts/mcp_server.py` for Claude Desktop. Setup + caveats: `docs/08-mcp-server.md`.
- **AI shopping assistant (optional, needs `AI__ENABLED` and an LLM).** `core/llm.py` is the chat-model provider factory (Ollama by default via `LLM__PROVIDER` / `LLM__OLLAMA_MODEL`, Anthropic optional) plus `expand_query`; `services/rag.py` is a LangGraph retrieve-then-generate graph; `services/ai_assistant.py` streams the reply as SSE and persists turns; `routes/ai.py` exposes `POST /ai/chat` (SSE) and the conversation endpoints. It grounds on the vector store and cites only recommended listings. Design and tuning: `docs/07-ai-assistant.md`.
